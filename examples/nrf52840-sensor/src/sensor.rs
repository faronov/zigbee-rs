...
//!
//! Uses the shared `zigbee-bme280` / `zigbee-sht3x` drivers (address,
//! register map, reset, oversampling, and CRC handling exactly matching
//! this firmware's previous hand-written `bme280.rs`/`sht31.rs`) over the
//! board's async TWISPI0 bus, following the same recoverable
//! probe-then-read pattern as `examples/efr32mg1-sensor/src/sensor.rs`.
//!
//! Only one of `sensor-bme280` / `sensor-sht31` is expected to be enabled;
//! if both are, BME280 takes priority (matching the previous
//! `I2C_SENSOR_ADDR` selection in `main.rs`).
...
mod bme280_sensor {
    ...
    use nrf52840_dk::SensorI2c;
    use nrf52840_sensor_product::profile::pressure_pa_to_zcl;
    use zigbee_bme280::asynch::Bme280;
    use zigbee_bme280::{Chip, Oversampling, SamplingConfig, PRIMARY_ADDRESS};

    /// x8 oversampling on all three channels, matching the original
    /// firmware's `ctrl_hum = 0b100` / `ctrl_meas = 0b1001_0001`.
...
mod sht31_sensor {
    ...
    impl<'d> Sensor<'d> {
        ...
        /// retried, matching the original firmware.
