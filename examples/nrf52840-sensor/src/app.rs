//! nRF52840 sensor application state machine.
//!
//! Extracted unchanged in behavior from the single-file `main.rs` this
//! firmware used before: the same bounded (four-round) MAC receive/poll
//! window, the same two-level fast/slow polling with a post-join fast-poll
//! window ended early by a real `ConfigureReporting` from the coordinator,
//! the same Device_annce retry policy (skipped only for the very first
//! cold-boot silent resume — see [`SensorApp::cold_start`]), the same
//! sensor/report cadence, button behavior, and durable checkpointing at the
//! same points in the lifecycle.
//!
...
use nrf52840_sensor_product::storage::SecurityStore;
use nrf52840_sensor_product::ENDPOINT;
...
/// Radio + RNG-backed MAC driver this product uses.
pub type SensorMac =
    NrfMac<'static, embassy_nrf::peripherals::RADIO, embassy_nrf::peripherals::RNG>;

type SensorNode<'a> = ZigbeeNode<'a, SensorMac, SecurityStore<'static>, SensorProfile>;

pub(crate) fn persistence_failure(error: SecurityStoreError) -> ! {
    match error {
...
pub struct SensorApp<'a> {
    ...
    #[cfg(not(any(feature = "sensor-bme280", feature = "sensor-sht31")))]
    temp_sensor: Temp<'static>,
    #[cfg(not(any(feature = "sensor-bme280", feature = "sensor-sht31")))]
    hum_tick: u32,
    #[cfg(any(feature = "sensor-bme280", feature = "sensor-sht31"))]
    env_sensor: sensor::Sensor<'static>,
...
impl<'a> SensorApp<'a> {
    ...
    pub fn new(
        ...
        led: gpio::Output<'static>,
        button: gpio::Input<'static>,
        #[cfg(not(any(feature = "sensor-bme280", feature = "sensor-sht31")))] temp_sensor: Temp<
            'static,
        >,
        #[cfg(any(feature = "sensor-bme280", feature = "sensor-sht31"))] env_sensor: sensor::Sensor<
            'static,
        >,
        saadc: Saadc<'static, 1>,
    ) -> Self {
    ...
    async fn read_sensors(&mut self) {
        ...
        #[cfg(any(feature = "sensor-bme280", feature = "sensor-sht31"))]
        {
            if let Some(reading) = self.env_sensor.sample().await {
        ...
        #[cfg(not(any(feature = "sensor-bme280", feature = "sensor-sht31")))]
        {
            let raw_temp = self.temp_sensor.read().await;
            let temp_hundredths = (raw_temp.to_bits() * 100 / 4) as i16;
            self.hum_tick = self.hum_tick.wrapping_add(1);
            let hum_hundredths = 5000u16 + ((self.hum_tick % 100) as u16).wrapping_mul(10);
            environment.update_environment(TemperatureHumidityMeasurement {
                ...
                Ok(Some(indication)) => {
                    ...
                    if !self.interview_done && self.node.reporting_is_configured() {
                        info!(
                            ...
                            self.node.device().configured_cluster_count(ENDPOINT),
                            self.node.profile().expected_report_clusters()
                        );
                        self.fast_poll_until = Instant::now() + Duration::from_secs(5);
                        self.interview_done = true;
                        self.led.set_high(); // OFF — power save
                    }
    ...
    async fn service_periodic_joined(&mut self, now: Instant) {
        ...
        // Identify LED blink.
        if self.node.device().is_identifying(ENDPOINT) {
            self.led.toggle();
        }
    ...
    pub async fn run(&mut self) -> ! {
        ...
        loop {
            ...
            if self.was_fast_polling && !in_fast_poll {
                let cfg = self.node.device().configured_cluster_count(ENDPOINT);
                info!(
