...
async fn main(_spawner: Spawner) {
    ...
    if woke_from_sleep {
        ...
        phy6222_hal::flash::release_deep_sleep(); // wake flash from deep power-down
    ...
    loop {
        ...
        if !in_fast_poll && device.is_joined() && poll_ms >= 5000 {
            ...
            device.save_state(&mut nv);
            // Prepare peripherals for minimum power
            phy6222_hal::gpio::prepare_for_sleep(1 << pins::BTN); // keep button pull-up
            phy6222_hal::flash::enter_deep_sleep(); // flash to ~1µA
            ...
            log::info!("[PHY6222] System sleep ({}s) — radio+flash off", poll_ms / 1000);
                        ...
                        if let Some(ev) = device.process_incoming(&ind, &mut cls).await {
                            match &ev {
                                StackEvent::RejoinRequested => {
                                    log::info!("[PHY6222] Secure rejoin requested");
                                    if device.secure_rejoin().await.is_ok() {
                                        ...
                                        log::warn!("[PHY6222] Secure rejoin failed");
                                    }
                                    break;
                                }
...
/// Suppresses unchanged reports to save TX power.
fn setup_default_reporting(device: &mut ZigbeeDevice<Phy6222Mac>) {
    ...
    log::info!("[PHY6222] Default reporting configured (with change thresholds)");
}
