...
impl RtcClock {
    ...
    pub(crate) fn calibrate_internal(mut cal_clk: RtcCalSel, slowclk_cycles: u32) -> u32 {
        ...
        // H2 ECO2's RC_FAST calibration divider belongs to REF_TICK.
        // ESP-IDF v5.5.1 esp32h2/rtc_time.c enables it for the measurement.
        #[cfg(esp32h2)]
        let saved_tick_enable = PCR::regs().ctrl_tick_conf().read().tick_enable().bit();
        #[cfg(esp32h2)]
        if cal_clk == RtcCalSel::RcFast
            && crate::rtc_cntl::sleep::retained::divided_fast_clock(true, Efuse::chip_revision())
        {
            PCR::regs()
                .ctrl_tick_conf()
        ...
        let cal_val = loop {
            if timg0.rtccalicfg().read().rtc_cali_rdy().bit_is_set() {
                ...
                if crate::rtc_cntl::sleep::retained::divided_fast_clock(
                    cfg!(esp32h2),
                    Efuse::chip_revision(),
                ) && cal_clk == RtcCalSel::RcFast
                {
        ...
        PCR::regs()
            .ctrl_tick_conf()
            .modify(|_, w| w.tick_enable().bit(saved_tick_enable));
    ...
    pub(crate) fn calibrate(cal_clk: RtcCalSel, slowclk_cycles: u32) -> u32 {
        ...
        #[cfg(any(esp32c6, esp32h2))]
        let slowclk_cycles = if crate::rtc_cntl::sleep::retained::divided_fast_clock(
            cfg!(esp32h2),
            Efuse::chip_revision(),
        ) && cal_clk == RtcCalSel::RcFast
        {
