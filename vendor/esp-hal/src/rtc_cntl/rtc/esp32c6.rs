...
impl SavedClockConfig {
    pub(crate) fn save() -> Self {
        ...
        SavedClockConfig {
            ...
            source_freq_mhz,
            div,
            cpu_conf: PCR::regs().cpu_freq_conf().read().bits(),
            ahb_conf: PCR::regs().ahb_freq_conf().read().bits(),
            mspi_conf: PCR::regs().mspi_clk_conf().read().bits(),
        }
    }
    ...
    pub(crate) fn restore(self) {
        if self.source == CpuClockSource::Pll {
            rtc_clk_bbpll_enable();
            esp32c6_rtc_bbpll_configure_raw(RtcClock::xtal_freq().mhz(), self.source_freq_mhz);
        } else if self.source == CpuClockSource::RcFast {
            esp32c6_rtc_update_to_8m();
        }
        // Restore inactive divider banks too: entry overwrites CPU/AHB LS
        // dividers even when the saved source was PLL. Keep the MSPI divider
        // that the running application's flash timing was configured for.
