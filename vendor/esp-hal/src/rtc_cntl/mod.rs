...
impl<'d> Rtc<'d> {
    ...
    /// Retained-domain light sleep for the local C6/H2 backport.
    ///
    /// Only timer and digital GPIO wake sources are accepted. CPU, RAM, flash,
    /// peripheral and oscillator power domains remain enabled. BBPLL may stop;
    /// radio and other PLL consumers must be quiescent.
    ///
    /// # Safety
    ///
    /// Quiesce UART/USB/DMA/flash operations and other PLL consumers first.
    /// Watchdog deadlines must permit the sleep. No other CPU/RTC/PMU clock
    /// user may change their configuration until this function returns.
    ///
    /// This does not compensate SYSTIMER or rearm alarms. The caller must hold
    /// its critical section through clock compensation and alarm servicing.
    /// Returning `Ok` includes hardware rejection; inspect PMU wake/reject status.
    #[cfg(any(esp32c6, esp32h2))]
    pub unsafe fn sleep_light_retained(
        &mut self,
        wake_sources: &[&dyn WakeSource],
    ) -> Result<(), sleep::RetainedSleepError> {
        ...
        // Timer/digital GPIO do not configure RTC pads. In particular, do not
        // run C6's legacy EXT1 cleanup on unrelated, previously configured pads.
        config.start_sleep_checked(triggers)
    }
