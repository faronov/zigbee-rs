...
use super::retained::{RetainedSleepError, calibrated_period, divided_fast_clock, timer_ticks};
use crate::{
    efuse::Efuse,
    rtc_cntl::{
        Rtc,
...
impl WakeSource for TimerWakeupSource {
    fn apply(
        ...
        let ticks = unwrap!(timer_ticks(self.duration.as_micros(), unsafe {
            lp_aon().store1().read().bits()
        }));
        // "alarm" time in slow rtc ticks
        let now = rtc.time_since_boot_raw();
        let time_in_ticks = (now + ticks) & ((1 << 48) - 1);
        unsafe {
            ...
            if !self.deep_sleep {
                pmu().hp_sleep_hp_sys_cntl().modify(|_, w| {
                    w.hp_sleep_dig_pad_slp_sel()
                        ...
                        .bit(self.syscntl.dig_cpu_stall())
                        .hp_sleep_dig_pause_wdt()
                        .bit(self.syscntl.dig_pause_wdt())
                });
            }
        };
    }
...
impl PowerSleepConfig {
    ...
    fn apply_flags(&mut self, pd_flags: PowerDownFlags) {
        ...
        self.lp_sys_sleep.clk_power.set_xpd_rc32k(true);
        self.lp_sys_sleep
            .clk_power
...
impl SleepTimeConfig {
    ...
    fn rtc_clk_cal_fast(mut slowclk_cycles: u32) -> Result<u32, RetainedSleepError> {
        let xtal_freq = 32;
        ...
        if divided_fast_clock(true, Efuse::chip_revision()) {
            slowclk_cycles /= 32;
        }

        let xtal_cycles = RtcClock::calibrate_internal(RtcCalSel::RcFast, slowclk_cycles);
        calibrated_period(xtal_cycles, xtal_freq, slowclk_cycles)
    }

    fn new() -> Result<Self, RetainedSleepError> {
    ...
    fn light_sleep(pd_flags: PowerDownFlags) -> Result<Self, RetainedSleepError> {
...
impl RtcSleepConfig {
    ...
    ///
    /// This backport rejects non-retained configurations before changing clocks.
    pub(crate) fn start_sleep(&self, wakeup_triggers: WakeTriggers) {
        unwrap!(self.start_sleep_checked(wakeup_triggers));
    }

    pub(crate) fn start_sleep_checked(
        &self,
        wakeup_triggers: WakeTriggers,
    ) -> Result<(), RetainedSleepError> {
        super::retained::validate_retained(self.deep, self.pd_flags.0, wakeup_triggers.0)?;
        ...
        let clock = SavedClockConfig::try_save().ok_or(RetainedSleepError::UnsupportedClock)?;
        rtc_clk_cpu_freq_set_xtal();

        // misc_modules_sleep_prepare
        // TODO: IDF-7370
        let saradc = unsafe { &*esp32h2::APB_SARADC::ptr() };
        let adc_power_detect = saradc.ctrl().read().saradc2_pwdet_drv().bit();
        saradc
            .ctrl()
        ...
        let config = match SleepTimeConfig::light_sleep(self.pd_flags) {
            Ok(config) => config,
            Err(error) => {
                saradc
                    .ctrl()
                    .modify(|_, w| w.saradc2_pwdet_drv().bit(adc_power_detect));
                clock.restore();
                return Err(error);
            }
        };
