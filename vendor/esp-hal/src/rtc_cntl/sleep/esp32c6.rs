...
impl WakeSource for TimerWakeupSource {
    fn apply(
        ...
        let ticks = unwrap!(super::retained::timer_ticks(
            self.duration.as_micros(),
            unsafe { lp_aon().store1().read().bits() },
        ));
...
impl SleepTimeConfig {
    ...
    fn rtc_clk_cal_fast(mut slowclk_cycles: u32) -> Result<u32, super::RetainedSleepError> {
        ...
        if super::retained::divided_fast_clock(false, Efuse::chip_revision()) {
            slowclk_cycles /= 32;
        }

        let xtal_cycles = RtcClock::calibrate_internal(RtcCalSel::RcFast, slowclk_cycles);
        super::retained::calibrated_period(xtal_cycles, xtal_freq, slowclk_cycles)
    }

    fn new(_deep: bool) -> Result<Self, super::RetainedSleepError> {
        ...
        if slowclk_period == 0 {
            return Err(super::RetainedSleepError::InvalidCalibration);
        }
    ...
    fn light_sleep(pd_flags: PowerDownFlags) -> Result<Self, super::RetainedSleepError> {
    ...
    fn deep_sleep() -> Result<Self, super::RetainedSleepError> {
...
impl RtcSleepConfig {
    ...
    fn start_sleep_inner(
        ...
        if crate::rtc_cntl::rtc::CpuClockSource::current().is_err() {
            return Err(super::RetainedSleepError::UnsupportedClock);
        }
