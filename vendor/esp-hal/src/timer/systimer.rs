...
impl Unit {
    ...
    fn set_count(&self, value: u64) {
        ...
        let (high, low) = counter_words::split(value);
        #[cfg(not(esp32s2))]
        {
            let unitload = systimer.unitload(self.channel() as _);
            let unit_load = systimer.unit_load(self.channel() as _);

            unitload.hi().write(|w| w.load_hi().set(high));
            unitload.lo().write(|w| w.load_lo().set(low));

            unit_load.write(|w| w.load().set_bit());
        }
        #[cfg(esp32s2)]
        {
            systimer.load_hi().write(|w| w.load_hi().set(high));
            systimer.load_lo().write(|w| w.load_lo().set(low));
