...
pub enum Error {
    ...
    BadInput,
    /// A previous transmission or hardware ACK is still active.
    Busy,
    /// Resume the radio before transmitting.
    Sleeping,
}
...
impl core::fmt::Display for Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Error::Busy => write!(f, "Radio busy."),
            Error::Sleeping => write!(f, "Radio suspended."),
        }
    }
}
...
pub struct Ieee802154<'a> {
    ...
    phy_init_guard: Option<PhyInitGuard<'a>>,
}
...
impl<'a> Ieee802154<'a> {
    ...
    /// NOTE: Coexistence with Wi-Fi or Bluetooth is currently not possible. If you do it anyway,
    /// things will break.
    pub fn new(mut radio: IEEE802154<'a>) -> Self {
        let (_phy_clock_guard, phy_init_guard) = esp_ieee802154_enable(&mut radio);
        Self {
            _align: 0,
            transmit_buffer: [0u8; FRAME_SIZE],
            radio,
            _phy_clock_guard,
            phy_init_guard: Some(phy_init_guard),
        }
    }
    ...
    /// Stop a quiet MAC, retaining its registers, software queues and configuration.
    ///
    /// A queued/raced receive, active TX/ACK, or pending interrupt vetoes sleep.
    /// On a veto the PHY remains enabled. CPU/top/modem register domains must
    /// remain retained until `resume`; this does not support reset-on-wake sleep.
    pub fn try_suspend(&mut self) -> Result<(), SuspendError> {
        if self.phy_init_guard.is_none() {
            return Ok(());
        }
        raw::try_suspend()?;
        self.phy_init_guard.take();
        Ok(())
    }

    /// Restore the PHY and receiver after retained sleep, including rejected sleep.
    pub fn resume(&mut self) {
        if self.phy_init_guard.is_none() {
            self.phy_init_guard = Some(self.radio.enable_phy());
            raw::resume();
        }
    }

    /// Stop an outstanding application TX before its buffer can be reused.
    /// The caller must report this cancellation as a failed transmission.
    ...
    pub fn transmit(&mut self, frame: &Frame) -> Result<(), Error> {
        ...
        ieee802154_transmit(self.transmit_buffer.as_ptr(), false)
    }

    /// Transmit a raw frame
    pub fn transmit_raw(&mut self, frame: &[u8]) -> Result<(), Error> {
        raw::can_transmit()?;
        if frame.len() > self.transmit_buffer.len() - 1 {
