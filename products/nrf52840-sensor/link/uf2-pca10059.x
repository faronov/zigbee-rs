...
ASSERT(ORIGIN(MBR) + LENGTH(MBR) == ORIGIN(FLASH),
  "PCA10059 UF2 application must preserve the MBR page");
ASSERT(ORIGIN(FLASH) + LENGTH(FLASH) == ORIGIN(SECURITY_JOURNAL),
  "PCA10059 UF2 application overlaps its security journal");
ASSERT(LENGTH(SECURITY_JOURNAL) == 8K,
  "PCA10059 UF2 security journal must remain two pages");
ASSERT(ORIGIN(SECURITY_JOURNAL) + LENGTH(SECURITY_JOURNAL) == ORIGIN(BOOTLOADER),
  "PCA10059 UF2 journal must stop at the 0xF4000 bootloader");
ASSERT(ORIGIN(BOOTLOADER) + LENGTH(BOOTLOADER) == ORIGIN(BOOT_STATE),
  "PCA10059 UF2 bootloader/config must remain below MBR params");
