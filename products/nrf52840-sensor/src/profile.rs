...
//!
//! Reuses the shared [`TemperatureHumidityBattery`] archetype from
//! `zigbee-runtime`, matching the endpoint/cluster/reporting configuration
//! previously hand-assembled in `examples/nrf52840-sensor`'s `main.rs`
//! exactly. With the `sensor-bme280` feature, a Pressure Measurement
//! cluster is added via [`TemperatureHumidityBattery::with_pressure`],
//! which returns the distinct [`TemperatureHumidityPressureBattery`] type
//! rather than an optional field — see that type's doc comment in
//! `zigbee-runtime` for why. `SensorProfile` therefore selects its
//! component type per-feature below, so a build without `sensor-bme280`
//! never links the Pressure Measurement cluster at all. Pressure's
//! reporting is intentionally left to the coordinator's own
//! `ConfigureReporting` during interview, matching the original firmware.
