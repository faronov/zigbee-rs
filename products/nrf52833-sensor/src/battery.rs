...
//! measurement.
//!
//! Same electrical path and same cell chemistry as
//! `products/nrf52840-sensor`: the SAADC samples the internal VDD divider
//! with the crate's default `saadc::Config` (12-bit, ~3.6 V full scale),
//! and percentage is a linear 1.8 V (empty) .. 3.0 V (full) curve.
//!
//! This is declared per-product on purpose. Battery chemistry is a product
//! decision (see the architecture rules in `docs/book`), so a future
//! nRF52833 build with a different cell can change this curve without
//! touching the proven nRF52840 firmware.
...
mod tests {
    ...
    #[test]
    fn millivolts_matches_saadc_scaling() {
