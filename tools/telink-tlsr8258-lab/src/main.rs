...
//! cd zigbee-rs-fork/tools/telink-tlsr8258-lab
...
#[cfg(all(feature = "diag-smoke", feature = "sensor"))]
compile_error!(
    "feature `diag-smoke` is mutually exclusive with `sensor`; \
     build with `--no-default-features --features diag-smoke`"
);

#[cfg(all(feature = "diag-pm", feature = "sensor"))]
compile_error!(
...
use zigbee_aps::security::{
    ApsKeyType, ApsSecurity, ApsSecurityHeader, KEY_ID_DATA_KEY, KEY_ID_KEY_LOAD,
    KEY_ID_KEY_TRANSPORT, SEC_LEVEL_ENC_MIC_32, derive_key_load_key, derive_key_transport_key,
...
fn mark_bytes_as_words(base: u32, data: &[u8]) {
    ...
    while idx < data.len() && idx < 64 {
        ...
        mark32(base + idx as u32, u32::from_le_bytes([b0, b1, b2, b3]));
        idx += 4;
    }
}
...
mod radio {
    ...
    pub fn set_channel(channel: u8) {
        ...
        unsafe {
            ...
            core::ptr::write_volatile(REG_MODEM_BAND, (existing_band & 0xC3) | (band & 0x3C));
        }
...
impl Tlsr8258Mac {
    ...
    fn maybe_enqueue_pending(&mut self, data: &[u8], lqi: u8) {
        ...
        if !address_matches(&dst, self.pan_id, self.short_address, self.extended_address) {
            unsafe {
    ...
    fn receive_raw(&mut self, timeout_loops: u32) -> Result<RxPacket, MacError> {
        ...
        if was_tk_candidate {
            ...
            if buf.extend_from_slice(&pkt.data[..pkt.len]).is_ok() {
                match self.pending_rx.push_back(PendingPacket {
                    data: buf,
                    lqi: pkt.lqi,
                }) {
                    Ok(()) => unsafe {
    ...
    fn build_assoc_request(
        ...
        unsafe {
            ...
            core::ptr::write_volatile((DBG_MODE_BASE + 0x1E4) as *mut u32, 0xCAFE_BABE);
    ...
    fn csma_transmit(&mut self, frame: &[u8], ack_requested: bool) -> Result<(), MacError> {
        ...
        for attempt in 0..=max_retries {
            ...
            match self.receive_raw(ACK_WAIT_LOOPS) {
                ...
                Err(_) => {
                    mark32(DBG_MODE_BASE + 0x94, 0xC50A5000 | (attempt as u32));
                }
            }
            if attempt == max_retries {
...
impl MacDriver for Tlsr8258Mac {
    async fn mlme_scan(&mut self, req: MlmeScanRequest) -> Result<MlmeScanConfirm, MacError> {
        ...
        STICKY_TOTAL_PANS.store(
            prev_total.wrapping_add(pan_descriptors.len() as u32),
            Ordering::Relaxed,
        );
        let prev_permit = STICKY_PERMIT_PANS.load(Ordering::Relaxed);
        STICKY_PERMIT_PANS.store(
            prev_permit.wrapping_add(permit_joining_count),
            Ordering::Relaxed,
        );
        mark32(
            DBG_MODE_BASE + 0x1C8,
            STICKY_PERMIT_PANS.load(Ordering::Relaxed),
        );
        mark32(
            DBG_MODE_BASE + 0x1CC,
            STICKY_TOTAL_PANS.load(Ordering::Relaxed),
        );
        ...
        for poll_attempt in 0..6u8 {
            ...
            match self.transmit_raw(&data_req) {
                Ok(()) => {
                    ...
                    mark32(
                        DBG_MODE_BASE + 0x5C,
                        0xA55C0003 | ((poll_attempt as u32) << 8),
                    );
                }
                Err(_) => {
    ...
    async fn mlme_get(
        &self,
        attr: zigbee_mac::PibAttribute,
    ) -> Result<zigbee_mac::PibValue, MacError> {
    ...
    async fn mlme_poll(&mut self) -> Result<Option<MacFrame>, MacError> {
        ...
        for _ in 0..8 {
            match self.receive_raw(POLL_RESPONSE_LOOPS / 8) {
                Ok(pkt) => {
                    ...
                    if payload.len() >= 4 {
                        let nwk_dst = u16::from_le_bytes([payload[2], payload[3]]);
                        let our_short = self.short_address.0;
                        unsafe {
                            if nwk_dst == our_short && our_short != 0xFFFF {
                                let to_us_p = (DBG_JOIN_BASE + 0xD0) as *mut u32;
                                core::ptr::write_volatile(
                                    to_us_p,
                                    core::ptr::read_volatile(to_us_p).wrapping_add(1),
                                );
                                let first_to_us = (DBG_JOIN_BASE + 0xD8) as *mut u32;
                                if core::ptr::read_volatile(first_to_us) == 0 {
                                    ...
                                    for i in 0..8usize {
                                        let off = i * 4;
                                        let b0 = payload.get(off).copied().unwrap_or(0) as u32;
                                        let b1 = payload.get(off + 1).copied().unwrap_or(0) as u32;
                                        let b2 = payload.get(off + 2).copied().unwrap_or(0) as u32;
                                        let b3 = payload.get(off + 3).copied().unwrap_or(0) as u32;
                                        let word = b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
                                        let p_word =
                                            (DBG_JOIN_BASE + 0xDC + (i as u32 * 4)) as *mut u32;
                                        core::ptr::write_volatile(p_word, word);
                                    }
                                }
                            } else if nwk_dst >= 0xFFFC {
                                let bcast_p = (DBG_JOIN_BASE + 0xD4) as *mut u32;
                                core::ptr::write_volatile(
                                    bcast_p,
                                    core::ptr::read_volatile(bcast_p).wrapping_add(1),
                                );
                            }
                        }
                    }
        ...
        unsafe {
            ...
            core::ptr::write_volatile(none_p, core::ptr::read_volatile(none_p).wrapping_add(1));
            // Last-poll-stats: filtered_out count
            mark32(
                DBG_JOIN_BASE + 0xA8,
                (frames_in_loop << 16) | (filtered_out & 0xFFFF),
            );
        }
        Ok(None)
    }
    ...
    async fn mcps_data_indication_timeout(
        ...
        loop {
            ...
            if !self.promiscuous
                && !address_matches(&dst, self.pan_id, self.short_address, self.extended_address)
            {
                continue;
            }
            if frame_type != 1 {
...
fn handle_rf_rx_irq() {
    ...
    if total_len > 0 && total_len <= 130 {
        let status_byte = unsafe { core::ptr::read_volatile(rx_buf.add(total_len as usize + 3)) };
    ...
    if total_len > 0 && radio::packet_length_ok(rx_buf) && radio::packet_crc_ok(rx_buf) {
        ...
        if (2..=MAX_MAC_FRAME_LEN + 2).contains(&phy_len) {
            ...
            if let Some(seq) =
                rx_frame_needs_ack(rx_buf, frame_len, pan_id, short_address, ext_addr)
            {
...
fn receive_packet(
    ...
    for _ in 0..timeout_loops {
        ...
        if radio::rx_done() {
            ...
            mark32(
                DBG_MODE_BASE + 0x34,
                (total_len as u32) | ((payload_len as u32) << 8) | ((status_byte as u32) << 16),
            );
...
fn build_long_tx_probe(seq: u8) -> [u8; 17] {
    [
        0x41, 0x88, seq, 0x34, 0x12, 0x78, 0x56, 0x34, 0x12, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03,
        0x02, 0x01,
    ]
}
...
mod async_timer {
    ...
    pub fn on_alarm_irq() {
        super::TIMER0_ALARM_PENDING.store(true, core::sync::atomic::Ordering::Release);
        ...
        let prev = super::TIMER0_IRQ_COUNT.load(core::sync::atomic::Ordering::Relaxed);
        super::TIMER0_IRQ_COUNT.store(prev.wrapping_add(1), core::sync::atomic::Ordering::Release);
...
async fn send_device_annce(
    ...
    *aps_seq = aps_seq.wrapping_add(1);
    ...
    let total = if nwk_security.active_key().is_some() {
        ...
        let Some(encrypted) = nwk_security.encrypt(
            &payload[..aad_len],
            &aps_plain[..aps_total],
            &key_entry.key,
            &sec_hdr,
...
fn build_zdo_response(
    ...
    match cluster_id {
        ...
        ZDO_IEEE_ADDR_REQ => {
            ...
            out.get_mut(off..off + 8)?.copy_from_slice(if matched {
                &ieee_addr
            } else {
                &[0u8; 8]
            });
        ...
        ZDO_NODE_DESC_REQ => {
            ...
            push_le16(out, &mut off, SENSOR_STACK_COMPLIANCE_REVISION << 9)?; // Server mask: stack compliance revision.
...
fn decrypt_sensor_aps_payload(
    ...
    patched_aad[aps_header_len] = (patched_aad[aps_header_len] & !0x07) | SEC_LEVEL_ENC_MIC_32;
...
fn sensor_nv_checksum(data: &[u8]) -> u8 {
    data.iter()
        .fold(0xA5, |acc, byte| acc.wrapping_add(*byte).rotate_left(1))
}
...
fn bump_nwk_counter(mac: &Tlsr8258Mac, nwk_security: &NwkSecurity, nwk_frame_counter: &mut u32) {
    *nwk_frame_counter = nwk_frame_counter.wrapping_add(1);
    ...
    let Some(key_entry) = nwk_security.active_key() else {
        return;
    };
...
#[cfg(feature = "sensor")]
#[inline(never)]
fn persist_network_state(key: &[u8; 16], key_seq: u8, next_frame_counter: u32, pan: PanIdentity) {
...
fn load_persisted_network_state(nwk_security: &mut NwkSecurity) -> Option<LoadedNvState> {
    ...
    let mut key = [0u8; 16];
    key.copy_from_slice(&record[6..22]);
    let stored_frame_counter = u32::from_le_bytes([record[22], record[23], record[24], record[25]]);
    let next_frame_counter = stored_frame_counter.max(SENSOR_NWK_COUNTER_RESERVE);
...
#[cfg(feature = "sensor")]
#[inline(never)]
fn persist_network_key(key: &[u8; 16], key_seq: u8, next_frame_counter: u32, pan: PanIdentity) {
    persist_network_state(
        key,
        key_seq,
        next_frame_counter.wrapping_add(SENSOR_NWK_COUNTER_RESERVE),
...
fn install_transport_key(
    ...
    if cmd_payload.len() < 17 {
        mark32(
            DBG_MODE_BASE + 0xB8,
            0xA75CFF10 | ((cmd_payload.len() as u32) << 16),
        );
        return false;
    }
    ...
    let key_seq = if cmd_payload.len() > 17 {
        cmd_payload[17]
    } else {
        0
    };
...
fn handle_sensor_frame(
    ...
    let aps_data = if nwk_header.frame_control.security {
        ...
        if let Some(key_entry) = nwk_security.key_by_seq(sec_hdr.key_seq_number).cloned() {
            ...
            let Some(plain) = decrypt_initial_nwk_payload(
                nwk_security,
                aps_security,
                data,
                nwk_len,
        ...
        && aps_header.frame_control.ack_request
    {
        match send_aps_ack_raw(
            ...
            Ok(()) => mark32(
                DBG_MODE_BASE + 0x98,
                0xA9000000 | aps_header.aps_counter as u32,
            ),
            Err(_) => mark32(
                DBG_MODE_BASE + 0x98,
                0xA9FF0000 | aps_header.aps_counter as u32,
            ),
        }
    }
    ...
    let Some((rsp_cluster, rsp_len)) = build_zdo_response(
        cluster_id,
        payload,
        mac.short_address,
    ...
    for attempt in 0..PASSIVE_RX_ATTEMPTS {
        match mac.receive_raw(PASSIVE_RX_LOOPS) {
            Ok(pkt) => {
                ...
                if let Some(frame) =
                    zigbee_mac::primitives::MacFrame::from_slice(&pkt.data[..pkt.len])
                {
        ...
        match executor::block_on(mac.mlme_scan(scan)) {
            Ok(confirm) if !confirm.pan_descriptors.is_empty() => {
                ...
                for (idx, candidate) in confirm.pan_descriptors.iter().enumerate() {
                    ...
                    let coordinator_bonus =
                        if candidate.superframe_spec.pan_coordinator || is_short_coord {
                            0xF000
                        } else {
                            0
                        };
                ...
                match executor::block_on(mac.mlme_associate(assoc)) {
                    Ok(confirm) if confirm.status == AssociationStatus::Success => {
                        ...
                        .is_ok();
                        if da_bcast {
                            bump_marker(DBG_MODE_BASE + 0x12C);
                        }
                        announced = da_bcast;
                        mark32(
                            DBG_MODE_BASE + 0x28,
...
// Kept as disabled source-level history for the legacy local MAC investigation.
#[cfg(any())]
#[inline(never)]
    ...
    loop {
        ...
        match executor::block_on(device.tick(1, &mut clusters)) {
            TickResult::Event(StackEvent::Joined {
                ...
                mark32(
                    DBG_MODE_BASE + 0x34,
                    short_address as u32 | ((pan_id as u32) << 16),
                );
                mark32(DBG_MODE_BASE + 0x38, channel as u32);
                if first_tick {
                    mark32(DBG_MODE_BASE + 0x9C, 0xF1559000);
                }
            }
            TickResult::Event(StackEvent::CommissioningComplete { success }) => {
                ...
                if first_tick {
                    mark32(
                        DBG_MODE_BASE + 0x9C,
                        if success { 0xF155CC01 } else { 0xF155FFFF },
                    );
                }
            }
            TickResult::Event(_) => {
                ...
                if first_tick {
                    mark32(DBG_MODE_BASE + 0x9C, 0xF1550002);
                }
            }
            TickResult::RunAgain(ms) => {
                ...
                if first_tick {
                    mark32(DBG_MODE_BASE + 0x9C, 0xF1550003);
                }
            }
            TickResult::Idle => {
                ...
                if first_tick {
                    mark32(DBG_MODE_BASE + 0x9C, 0xF1550004);
                }
            }
        }
        first_tick = false;
            ...
            for _ in 0..4u8 {
                match executor::block_on(device.poll()) {
                    Ok(Some(ind)) => {
                        ...
                        let _ = executor::block_on(device.process_incoming(&ind, &mut clusters));
...
fn diag_pm_fail(code: u32) -> ! {
    ...
    let _ = tlsr8258_hal::mmio::analog_write(tlsr8258_hal::pm::DEEP_ANA_REG1, (code >> 8) as u8);
...
fn diag_smoke_main() -> ! {
    ...
    const SMOKE_PATTERN: [u8; 16] = [
        0xA5, 0x5A, 0xDE, 0xAD, 0xBE, 0xEF, 0xCA, 0xFE, 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD,
        0xEF,
    ];
